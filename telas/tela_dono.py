...
class VisualizeDonoScr(tk.Toplevel):
    ...
    def initUI(self):
        ...
        dono = crud.select_record("DonoArte", "Codigo", str(self.codigo))

        # Listar informações da obra de arte uma por linha
        lbl_dono = ttk.Label(self, text="Informações sobre o Dono")
        lbl_dono.pack(fill=tk.X, padx=15, pady=5)
        
        labels = ["Codigo", "Pais", "Nome", "Tipo"]
...
class CreateDonoScr(tk.Toplevel):
    def __init__(self, master=None):
        super().__init__(master=master)
        self.initUI()
        self.master.iconify()
